<script lang='ts'>
  ...
  const dataPromise = database.ref(`/${raidRepo}`).once('value').then(snapshot => snapshot.val() as ViewRaidData)
      ...
      <div class="content">
        <h1>{data.title}</h1>
          <div class="total-stats">
            <div>
              <p><span class="emoji-stat">🚀</span>{`${Object.keys(data.contributors).length}`} Contributors</p>
              <p><span class="emoji-stat">💾</span>{data.commits} {data.commits === 1 ? 'Commit' : 'Commits'}</p>
              <p><span class="emoji-stat">📃</span>{data.changedFiles} Changed Files</p>
            </div>
            <div>
              <p><span class="emoji-stat">⚔️</span>+{data.additions}&emsp;-{data.deletions}</p>
              <p><span class="emoji-stat">🔥</span>{data.additions - data.deletions} Net Deletions</p>
            </div>
          </div>
